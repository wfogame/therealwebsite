...
function cj3LoadImpl(options)
{
	...
	import(cj3LoaderPath + "/cj3.js").then(function(module)
	{
		module.default({absPath:cj3LoaderPath + "/cj3.wasm"}).then(function(m)
		{
			cj3Module = m;
			...
			m.cj3Init(options, cj3LoaderPath, m).then(function()
			{
				resolveFunc();
...
{
	if(cj3Loaded == 0)
		cj3LoadImpl({version:8});
	else if(cj3Loaded == 1)
	{
		cj3Loaded = 2;
		// Initialize applet support
		cj3Module.cj3AppletInit().then(function()
		{
			cj3StartApplets();
...
function cj3InjectLoader(p)
{
	...
	if(document.getElementById("cjLoader"))
	{
		...
		if(self.hasOwnProperty("cj3AttachBodyObserver"))
			cj3AttachBodyObserver(/*appletRunnerMode*/true);
		return;
	}
...
function cheerpjInit(options)
{
	...
	if(self.window)
		cj3AttachBodyObserver(/*appletRunnerMode*/false);
